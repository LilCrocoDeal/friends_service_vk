...
После запуска также можно проверить написанные unit-тесты, введя последовательно в терминал
...
## OpenAPI
Основой для написания спецификации использовалась python-библиотека __drf-yasg__, которая предоставляет базовую самогенерирующуюся OpenAPI спецификацию.
Конечно, основные запросы были описаны дополнительно вручную. 
...
было решено сделать это подобным образом для того, чтобы отправитель запроса не спамил повторными заявками в друзья (если висит одна заявка в друзья, нельзя 
отправить еще одну тому же пользователю).
...
Чтобы избежать проблем с тем, что отклоненные заявки копятся в базе заявок, есть возможность по адресу `/requests/delete/` вручную удалить 
заявку от определенного пользователя (это было сделано в основном для удобства просмотра списка заявок и не очень функционально полезно в 
реальном смысле).
...
Регистрация и авторизация по токенам были написаны вручную на основе предоставляемых возможностей _rest_framework_.
...
В первом, самом простом тесте, просто проверяется, что пользователь успешно регистрируется в системе.

Второй тест проверяет условие, что отправление двумя пользователями запросов друг другу влечет их автоматическое добавление в друзья. В конце проверяется 
список друзей одного из пользователей.

В третьем тесте пользователь одобряет заявку в друзья, а потом обратно удаляет из друзей. Затем, заходя за второго пользователя, проверяется, что нельзя 
отправить повторный запрос в друзья одному и тому же пользователю, пока висит предыдущий.

Четвертый тест рассматривает всевозможные состояния отношений с пользователем во вкладке `/info`.

В последнем тесте четверо пользователей отправляют запросы пятому в друзья, а затем производятся разнообразные действия с этими заявками (отклонение, добавление, удаление и т.д.).
